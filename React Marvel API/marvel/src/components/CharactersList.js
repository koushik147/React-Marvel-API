...
import SearchShows from './CharactersSearch';
import 
{ Card,
 CardActionArea, 
 CardContent, 
...
const CharactersList = () => {
    ...
    useEffect(() => {
		...
		async function fetchData() {
			const pagevalue = parseInt(pageNo);
        try{
			...
			try {
                ...
                const totalpages = data.data.total / LIMIT;
		...
		() => {
			...
			async function fetchData() {
				try {
					...
					const { data } = await axios.get( url + "&nameStartsWith=" + searchTerm  + "&limit=" + LIMIT + "&offset=" + offset);
            ...
            <>
					...
					<div>
                        ...
                        <div>
                            ...
                            <br />
							<Button>
								<Link to = {`/characters/page/${parseInt(pageNo) - 1 < 0 ? 0 : parseInt(pageNo) - 1 }`} 
								    >
									PREVIOUS
								</Link>
							</Button>
                            { pageNo }
							<Button>
								<Link to = {`/characters/page/${parseInt(pageNo) + 1 > 51 ? 51 : parseInt(pageNo) + 1}`}
								    >
									Next
								</Link>
							</Button>
